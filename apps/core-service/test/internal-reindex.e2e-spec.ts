...
describe('Internal Reindex (e2e)', () => {
  ...
  it('GET /internal/subjects/:subjectId/documents requires API key and returns docs', async () => {
    ...
    const user = await prisma.user.findFirst({ where: { email: 'reindex_list@test.com' } })
    const docId = cuid()
    ...
    // 401 missing
    await request(app.getHttpServer()).get(`/internal/subjects/${subjectId}/documents`).expect(401)
    ...
    expect(ok.body.find((d: any) => d.id === docId && typeof d.s3Key === 'string')).toBeTruthy()
  })
  ...
  it('PUT /internal/reindex/:subjectId/chunks upserts chunks+embeddings idempotently and validates payload', async () => {
    ...
    const user = await prisma.user.findFirst({ where: { email: 'reindex_upsert@test.com' } })
    const docId = cuid()
    await prisma.document.create({
      data: {
        id: docId,
    ...
    const bodyBadDoc = {
      ...
      dim: 384,
      chunks: [
        { index: 0, text: 'Hello', embedding: Array(384).fill(0.1) },
      ],
    }
    await request(app.getHttpServer())
    ...
    const badDim = {
      ...
      dim: 384,
      chunks: [
        { index: 0, text: 'Hello', embedding: Array(10).fill(0.1) },
      ],
    }
    await request(app.getHttpServer())
      .put(`/internal/reindex/${subjectId}/chunks`)
    ...
    const makeBatch = (scale: number) => ({
      ...
      dim: 384,
      chunks: [
        { index: 0, text: 'A', embedding: Array(384).fill(0).map((_, i) => i * 0.001 * scale) },
    ...
    for (const ch of chunks) {
      const emb = await prisma.embedding.findUnique({ where: { chunkId: ch.id } })
