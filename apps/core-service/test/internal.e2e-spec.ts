...
describe('InternalController (e2e)', () => {
  ...
  it('PUT /internal/documents/:id/analysis enforces API key and document existence, then upserts analysis and marks COMPLETED', async () => {
    ...
    const me = await prisma.user.findFirst({ where: { email: 'internal_ok@test.com' } })
    ...
    await request(app.getHttpServer())
      ...
      .send({ engineVersion: 'oracle-v1', resultPayload: { keywords: [], metrics: { pages: 1, textLength: 1 } } })
      .expect(401)
    ...
    await request(app.getHttpServer())
      .put(`/internal/documents/${documentId}/analysis`)
      .set('X-Internal-API-Key', INTERNAL_KEY)
      .send({ engineVersion: 'oracle-v1', resultPayload: { keywords: [{ term: 'ai', score: 1 }], metrics: { pages: 3, textLength: 120 } } })
      .expect(200)

    const doc = await prisma.document.findUnique({ where: { id: documentId }, include: { analysisResult: true } })
