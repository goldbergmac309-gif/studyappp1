...
describe('Workspace (e2e)', () => {
  ...
  it('GET /workspace/personas should list seeded personas', async () => {
    const res = await request(app.getHttpServer()).get('/workspace/personas').expect(200)
    expect(Array.isArray(res.body)).toBe(true)
    // Expect at least our four seeds
    expect(res.body.length).toBeGreaterThanOrEqual(2)
    const names = (res.body as Array<{ name: string }>).map((p) => p.name)
    expect(names).toEqual(expect.arrayContaining(['STEM Lab', 'Humanities Hub']))
  })

  describe('Protected endpoints authz', () => {
    it('should 401 without JWT', async () => {
      await request(app.getHttpServer()).get('/subjects/some/workspace').expect(401)
      await request(app.getHttpServer()).post('/subjects/some/apply-persona').send({ personaId: 'x' }).expect(401)
      await request(app.getHttpServer()).patch('/subjects/some/workspace/layout').send({ widgets: [] }).expect(401)
    })
  })
  ...
  describe('Happy path + ownership + validation', () => {
    it('should apply persona to owned subject, then read and update layout; block re-apply and enforce ownership', async () => {
      ...
      const personasRes = await request(app.getHttpServer()).get('/workspace/personas').expect(200)
        ...
        widgets: [
          {
            ...
            size: { width: target.size.width + 1, height: target.size.height + 1 },
          },
        ],
      }
      ...
      await request(app.getHttpServer())
        .patch(`/subjects/${subjectId}/workspace/layout`)
        .set('Authorization', `Bearer ${tokenA}`)
        .send({ widgets: [{ id: 'nonexistent', position: { x: 0, y: 0 }, size: { width: 1, height: 1 } }] })
        .expect(400)
    })
  })
})
